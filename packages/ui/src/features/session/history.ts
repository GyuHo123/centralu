...
/**
 * 커서가 첫 줄에 있나 / 마지막 줄에 있나 — **개행 기준으로**.
 *
 * 여기서는 눈에 보이는 줄을 세지 않는다. 세려면 커서의 화면 좌표가 필요하고, 그건
 * DOM 없이 값으로 시험할 수 있는 판단이 아니다. 접힌 줄은 caret.ts가 거울로 재고,
 * SessionView가 둘을 겹쳐 쓴다 — 싼 판단으로 먼저 걸러야 비싼 측정이 드물어진다.
 *
 * (한동안은 개행만 셌다. 긴 한 줄이 접히면 두 번째 줄에서 누른 위 화살표가 커서가
 * 아니라 기록을 올렸고, 쓰던 글이 사라진 것처럼 보였다 — 사용자 지적 2026-09-07.)
 */
